...
# partial dataset, only take label 0 and 1
train_index = np.where(y_train <= 3)[0]
x_train = np.take(x_train, train_index, axis=0)
y_train = np.take(y_train, train_index, axis=0)

test_index = np.where(y_test <= 3)[0]
x_test = np.take(x_test, test_index, axis=0)
y_test = np.take(y_test, test_index, axis=0)
...
if args.resume:
    ...
    #model.add(layers.Dropout(0.5))
    #model.add(layers.BatchNormalization())
    model.add(layers.Dense(4, activation='softmax'))#, kernel_initializer='he_uniform'))
