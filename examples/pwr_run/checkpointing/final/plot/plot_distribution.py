...
fig.suptitle("CDF of normalized runtime (to baseline)")
...
for test in testcases1:
    ...
    axs.hist(sort, 100, label = test, density = True, cumulative = True, histtype='step')

axs.set_xlabel('reference JCT / scheme JCT') #'training time (h)')
axs.set_ylabel('CDF')
axs.legend(loc='center right')#, bbox_to_anchor=(1, 0.5))
axs.grid(which='both', axis='both', linestyle=':', color='black')

#plt.show()
plt.savefig('cdf/normalized_runtime_CDF_to_baseline.png')
