...
def random_promotion(K80_free, P100_free, V100_free, K80_promote_list, P100_promote_list, demote_list):
    num_promote = len(K80_promote_list) + len(P100_promote_list)

    V100_avail = V100_free + len(demote_list)
    V100_pool = list(set(demote_list).union(K80_promote_list).union(P100_promote_list))

    if V100_avail >= len(V100_pool): # all jobs in promote_list get promoted
        return K80_promote_list[:], P100_promote_list[:], [], []
    else:
        sorted_pool = random.sample(V100_pool, V100_avail)
        K80_promotion_list = list(set(K80_promote_list).intersection(sorted_pool))
        P100_promotion_list = list(set(P100_promote_list).intersection(sorted_pool))
        demotion_list = list(set(demote_list).difference(sorted_pool))
        P100_avail = P100_free + len(P100_promotion_list)
        if len(demotion_list) > P100_avail:
            P100_demotion_list = demotion_list[:P100_avail]
            K80_demotion_list = demotion_list[P100_avail:]
        else:
            P100_demotion_list = demotion_list
            K80_demotion_list = []
        return K80_promotion_list, P100_promotion_list, K80_demotion_list, P100_demotion_list
...
while True:
            ...
            if index < len(queue) and queue_dict[queue[index]] < time_passed: # make sure job has arrived in the queue
                ...
                for gpu, job in P100_job.items():
                    if job == 'idle': # schedule new job here if idle
                        ...
                        P100_job[gpu] = job_new # allocate gpu for it, but don't start yet
                        index += 1
                        P100_used += 1
                        break
    if K80_used < K80_cap:
    ...
    if len(K80_promote_list) > 0 or len(P100_promote_list) > 0:
        ...
        if len(K80_demoted) > 0:
            print('demoted to K80 jobs: ', K80_demoted)
        if len(P100_demoted) > 0:
            print('demoted to P100 jobs: ', P100_demoted)
        # stop all promoted jobs on K80 and P100
        ...
        V100_new_remain = list(set(demote_list).difference(total_demoted))
        # start remaining new jobs on V100, make sure the gpu equals its allocated one
        ...
        for job_new in new_pool[:]:
            for gpu, job in K80_job.items():
                if job == job_new: # if gpu idle, schedule new job here
                    ...
                    new_pool.remove(job_new)
                    break
        # start remaining new jobs on P100, make sure the gpu equals its allocated one
        for job_new in new_pool[:]:
            for gpu, job in P100_job.items():
                if job == job_new: # if gpu idle, schedule new job here
                    real_node, real_gpu = P100_LUT(gpu)
        ...
        if len(total_promoted) > 0 or len(total_demoted) > 0 or len(new_pool) > 0:
