...
speedup_dict_K80 = {}
for item in queue:
    speedup_dict_K80[str(item)] = 0 
speedup_dict_P100 = {}
for item in queue:
    speedup_dict_P100[str(item)] = 0 
predict_dict_K80 = {}
for item in queue:
    predict_dict_K80[str(item)] = 0 
predict_dict_P100 = {}
for item in queue:
...
def max_speedup_promotion_K80(K80_free, V100_free, K80_promote_list, demote_list):
    num_promote = len(K80_promote_list)
    global speedup_dict_K80

    # selectively promote among active V100 jobs and promote list jobs
    V100_pool = list(set(demote_list).union(K80_promote_list))       
    if num_promote <= V100_free: # promote all jobs as well
        return K80_promote_list[:], []
    else: # promote the top 4 jobs            
        pool_dict_K80 = {}
        V100_avail = V100_free + len(demote_list)
        for job in V100_pool:
            if job in speedup_dict_K80:
                pool_dict_K80[job] = speedup_dict_K80[job]        
        sorted_pool = sorted(pool_dict_K80, key=pool_dict_K80.get, reverse=True)[:V100_avail] 
        promotion_list = list(set(K80_promote_list).intersection(sorted_pool))                     
        ...
        # in sorted pool
        for job_demote in sorted(pool_dict_K80, key=pool_dict_K80.get, reverse=True):
            if job_demote in demotion_list:
                for job_promote in sorted(pool_dict_K80, key=pool_dict_K80.get, reverse=False):
...
def max_speedup_promotion_P100(P100_free, V100_free, P100_promote_list, demote_list):
    num_promote = len(P100_promote_list)
    global speedup_dict_P100

    # selectively promote among active V100 jobs and promote list jobs
    V100_pool = list(set(demote_list).union(P100_promote_list))       
    if num_promote <= V100_free: # promote all jobs as well
        return P100_promote_list[:], []
    else: # promote the top 4 jobs            
        pool_dict_P100 = {}
        V100_avail = V100_free + len(demote_list)
        for job in V100_pool:
            if job in speedup_dict_P100:
                pool_dict_P100[job] = speedup_dict_P100[job]        
        sorted_pool = sorted(pool_dict_P100, key=pool_dict_P100.get, reverse=True)[:V100_avail] 
        promotion_list = list(set(P100_promote_list).intersection(sorted_pool))                     
        demotion_list = list(set(demote_list).difference(sorted_pool))
        if 'idle' in demotion_list:
            demotion_list.remove('idle') # this includes force demotion
        ...
        # in sorted pool
        for job_demote in sorted(pool_dict_P100, key=pool_dict_P100.get, reverse=True):
            if job_demote in demotion_list:
                for job_promote in sorted(pool_dict_P100, key=pool_dict_P100.get, reverse=False):
                    if job_promote in promotion_list:
...
def min_speedup_demotion_P100(P100_promote_list, demote_list):
    ...
    for job_demote in sorted(pool_dict, key=pool_dict.get, reverse=True):
        if job_demote in demotion_list:
            for job_promote in sorted(pool_dict, key=pool_dict.get, reverse=False):
                if job_promote in promotion_list:
                    if speedup_dict_P100[job_promote] - speedup_dict_P100[job_demote] < 0.1: # use 0.1 for P100 pairs
                        demotion_list.remove(job_demote)
                        promotion_list.remove(job_promote)
                        break
...
def check_step2_complete_K80(job_list):
    log_path = '/scratch/li.baol/tsrbrd_log/job_runs/' + testcase + '/'
    global step1_job
    global step2_job_K80
    global V100_epoch_time
    global K80_epoch_time
    global speedup_dict_K80

    for job in job_list:
        if job in step1_job and job not in step2_job_K80 and job != 'idle':
            log_dir = log_path + 'job' + job + '/*'
            dirs = glob.glob(log_dir)
            dirs.sort()
            tc = ''
            ...
            if tc != '':
                ...
                try:
                    if len(iterator.Scalars(tag)) > 2: # this way we can collect one epoch time
                        ...
                        speedup_dict_K80[job] = speedup
                        step2_job_K80.append(job)
                        print('job' + job + ' has reached step2 complete on K80')
                except Exception:
                    pass

def check_step2_complete_P100(job_list):
    log_path = '/scratch/li.baol/tsrbrd_log/job_runs/' + testcase + '/'
    global step1_job
    global step2_job_P100
    global V100_epoch_time
    global P100_epoch_time
    global speedup_dict_P100

    for job in job_list:
        if job in step1_job and job not in step2_job_P100 and job != 'idle':
            log_dir = log_path + 'job' + job + '/*'
            dirs = glob.glob(log_dir)
            dirs.sort()
            tc = ''
            ...
            if tc != '':
                ...
                try:
                    if len(iterator.Scalars(tag)) > 2: # this way we can collect one epoch time
                        ...
                        V100_time_step2 = V100_epoch_time[job]
                        P100_time_step2 = wall_time[1] - wall_time[0]
                        speedup = (P100_time_step2 - V100_time_step2) / P100_time_step2
                        speedup_dict_P100[job] = speedup
                        step2_job_P100.append(job)
                        print('job' + job + ' has reached step2 complete on P100')
                except Exception:
                    pass
# measure job
...
while True:
    ...
    check_step2_complete_K80(list(K80_job.values()))   
    check_step2_complete_P100(list(P100_job.values()))   

    step2_job = list(set(step2_job_K80).union(step2_job_P100)) 
    ...
    for gpu, job in V100_job.items():
        if job not in qualified_job and job != 'idle':
            if job in step1_job:
                ...
                speedup_pred_K80 = model_K80.predict(np.array([x1, x2, x3]).reshape((1,-1)))[0] / 100
                speedup_dict_K80[job] = speedup_pred_K80
                predict_dict_K80[job] = speedup_pred_K80
                speedup_pred_P100 = model_P100.predict(np.array([x1, x2, x3]).reshape((1,-1)))[0] / 100
                speedup_dict_P100[job] = speedup_pred_P100
                predict_dict_P100[job] = speedup_pred_P100
...
##                print('printing v100_1st ' + job + ' for debugging purpose: ' + str(v100_1st[job]))
    ...
    if len(K80_promote_list) > 0 or len(P100_promote_list) > 0 or len(demote_list) > 0:
        if new_arrival == 0:
            demote_list_cpy = demote_list[:]
            K80_promoted, K80_demoted = max_speedup_promotion_K80(K80_free, V100_free, K80_promote_list, demote_list_cpy)
            if len(K80_demoted) > len(K80_promoted):
                print('should never happen for K80')
                pdb.set_trace()
            demote_list_cpy = list(set(demote_list_cpy).difference(K80_demoted))
            V100_free_cpy = V100_free - (len(K80_promoted) - len(K80_demoted))
            P100_promoted, P100_demoted = max_speedup_promotion_P100(P100_free, V100_free_cpy, P100_promote_list, demote_list_cpy)
            ...
            if (len(K80_demoted)+len(P100_demoted))-(len(K80_promoted)+len(P100_promoted)) > new_arrival - V100_free:
                ...
                str(new_arrival))
                print('original K80 demotion: ' + str(K80_demoted))
                print('original P100 demotion: ' + str(P100_demoted))

                K80_demoted_pool = {}
                for job in K80_demoted:
                    if job in speedup_dict_K80:
                        K80_demoted_pool[job] = speedup_dict_K80[job]
                P100_demoted_pool = {}
                for job in P100_demoted:
                    if job in speedup_dict_P100:
                        P100_demoted_pool[job] = speedup_dict_P100[job]

                have_to_demote = len(K80_promoted) + len(P100_promoted) + new_arrival - V100_free
                if have_to_demote > 0: 
                    # number of jobs that have to be demoted
                    if len(P100_demoted) >= have_to_demote:
                        K80_demoted = []
                        P100_demoted = sorted(P100_demoted_pool, key=P100_demoted_pool.get,
                        reverse=False)[:have_to_demote]
                    else:
                        K80_demoted = sorted(K80_demoted_pool, key=K80_demoted_pool.get,
                        reverse=False)[:(have_to_demote-len(P100_demoted))]
                       
                else:
                    # no jobs need to be demoted
                    K80_demoted = []
                    P100_demoted = []
                print('new K80 demotion: ' + str(K80_demoted))
                print('new P100 demotion: ' + str(P100_demoted))

        total_demoted = list(set(K80_demoted).union(P100_demoted))
        total_promoted = list(set(K80_promoted).union(P100_promoted))

        if len(K80_promoted) > 0:
            if new_arrival == 0:
                print('no new job arrivals')
            print('K80 promoted jobs: ', K80_promoted)
        if len(P100_promoted) > 0:
            if new_arrival == 0:
                print('no new job arrivals')
            print('P100 promoted jobs: ', P100_promoted)

        if len(K80_demoted) > 0:
            print('K80 demoted jobs: ', K80_demoted)
        if len(P100_demoted) > 0:
            print('P100 demoted jobs: ', P100_demoted)
        # stop all promoted jobs on K80
        checkpoint_finish_check = []
        for gpu, job in K80_job.items():
            if job in K80_promoted:
        ...
        # resume demoted jobs on P100, make sure the gpu is idle
        for job_new in P100_demoted[:]:
            if finish_dict['job'+job_new] != 1:
                for gpu, job in P100_job.items():
                    if job == 'idle': # if gpu idle, schedule new job here
                        real_node, real_gpu = P100_LUT(gpu)
...
speedup_name_K80 = 'speedup_K80.json'
speedup_name_P100 = 'speedup_P100.json'
predict_name_K80 = 'predict_K80.json'
...
with open(speedup_name_K80, 'w') as fp1:
   json.dump(speedup_dict_K80, fp1, sort_keys=True, indent=4)
with open(speedup_name_P100, 'w') as fp1:
   json.dump(speedup_dict_P100, fp1, sort_keys=True, indent=4)
with open(predict_name_K80, 'w') as fp1:
   json.dump(predict_dict_K80, fp1, sort_keys=True, indent=4)
