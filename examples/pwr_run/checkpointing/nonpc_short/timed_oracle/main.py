...
def max_speedup_to_idle(V100_free, promote_list):
    global speedup_dict
    pool_dict = {}
...
def max_speedup_one(K80_free, single_job, promote_list, force_demote):
    global speedup_dict
...
while True:
    ...
    promote_idle = max_speedup_to_idle(V100_free, promote_list)
    ...
    if len(epoch_end_job) > 0:
        ...
        if len(V100_epoch) > 0:
            promoted_job, demoted_job = max_speedup_one(K80_free, V100_epoch[0], promote_list_new, force_demote)
            ...
            # perform a check, make sure all promoted/demoted jobs are scheduled
            if len(promoted_job) > 0 or len(demoted_job) > 0 or len(new_pool) > 0:
                raise ValueError('Bug with promotion scheme, more jobs than free gpus')
