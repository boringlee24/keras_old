...
######################### do a regression fit ########################
with open('regression/speedup.json') as f:
    speedup_reg_dict = json.load(f)
with open('regression/max_pwr.json') as f:
...
for i in range(50):
   ...
   if max_pwr_dict[job_name] > 0:
       ...
       speedup.append(speedup_reg_dict[job])
x = np.array(max_pwr).reshape((-1, 1))
y = np.array(speedup)
                ...
                if job_demote in demotion_list:
                    for job_promote in sorted(pool_dict, key=pool_dict.get, reverse=False):
                        if job_promote in promotion_list:
                            value = (speedup_dict[job_promote] - speedup_dict[job_demote])*1800 - (ovhd_profile[job_promote] + ovhd_profile[job_demote] + 2 * 20)
                            if value < 0:
                    ...
                    if 'param' in data_str:
                        ...
                        if ovhd_start[job] != 0:
                            if ckpt_qual_dict[job_name] == 1:
                                ...
                                # if job in V100_job and ovhd not profiled
                                if job in V100_job.values() and ovhd_profile[job] == 0:
...
ovhd_profile_name = testcase + '_overhead_profile.json'
