...
nodelist='c2176,d1006'
testcase = 'random_practical_finish_2'
...
while True:
    ...
    if set(K80_run) != set(K80_job):
        ...
        stdout = subprocess.check_output(['squeue -u $USER --state=PENDING | awk \"/_K/\" | awk \'{print $3}\''], shell=True)
        stdout = str(stdout)
    ...
    if set(V100_run) != set(V100_job):
        # first get pending jobs, in case the job is still pending rather than finished
        stdout = subprocess.check_output(['squeue -u $USER --state=PENDING | awk \"/_V/\" | awk \'{print $3}\''], shell=True)
        stdout = str(stdout)
