...
# plot P100 energy vs K80 energy

fig, axs = plt.subplots(1, 1, gridspec_kw={'hspace': 0, 'wspace': 0}, figsize=(12,5))
fig.suptitle("P100 energy(kWh) vs K80 energy(kWh)")
...
for key in all_pwr:
    ...
    axs.scatter(K80_energy, P100_energy, label = key)

axs.set_xlabel('K80 energy (kWh)')
...
plt.savefig("./energy/P100_vs_K80.png")

# Now plot P100 power reduction(W) vs K80 power(W)

fig, axs = plt.subplots(1, 1, gridspec_kw={'hspace': 0, 'wspace': 0}, figsize=(12,5))
fig.suptitle("P100 energy reduction(kWh) vs K80 energy(kWh)")
NUM_COLORS = len(all_pwr)
cm = plt.get_cmap('tab20') #'gist_rainbow')
axs.set_prop_cycle(color=[cm(1.*i/NUM_COLORS) for i in range(NUM_COLORS)])
for key in all_pwr:
    K80_energy = all_pwr[key]['K80'] * all_time[key]['K80'] / 1000
    P100_energy = all_pwr[key]['P100'] * all_time[key]['P100'] / 1000
    axs.scatter(K80_energy, K80_energy - P100_energy, label = key)
...
plt.savefig("./energy/P100_reduction_vs_K80.png")

# Now plot P100 power save ratio (%) vs K80 power(W)

fig, axs = plt.subplots(1, 1, gridspec_kw={'hspace': 0, 'wspace': 0}, figsize=(12,5))
fig.suptitle("P100 energy save percentage (%) vs K80 energy(kWh)")
NUM_COLORS = len(all_pwr)
cm = plt.get_cmap('tab20') #'gist_rainbow')
axs.set_prop_cycle(color=[cm(1.*i/NUM_COLORS) for i in range(NUM_COLORS)])
for key in all_pwr:
    K80_energy = all_pwr[key]['K80'] * all_time[key]['K80'] / 1000
    P100_energy = all_pwr[key]['P100'] * all_time[key]['P100'] / 1000
    axs.scatter(K80_energy, (K80_energy - P100_energy) / K80_energy * 100, label = key)
...
plt.savefig("./energy/P100_save_vs_K80.png")
