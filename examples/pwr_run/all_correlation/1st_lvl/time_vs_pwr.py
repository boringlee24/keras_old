...
y1_data = []
y2_data = []
fig, axs = plt.subplots(1, 1, gridspec_kw={'hspace': 0, 'wspace': 0}, figsize=(12,8))
fig.suptitle("V100 time save percentage (%) vs V100 power(W)")
NUM_COLORS = len(all_pwr)
cm = plt.get_cmap('tab20') #'gist_rainbow')
axs.set_prop_cycle(color=[cm(1.*i/NUM_COLORS) for i in range(NUM_COLORS)])
for key in all_pwr:
...
#plt.show()
plt.savefig("./time_vs_power/V100_save_vs_V100.png")
