...
fig.suptitle(testcase +  " GPU utilization (%) during training epochs")
...
plt.savefig(base_dir + "util_png/" + testcase + '_util' + str(pwr) + ".png")

df = pandas.DataFrame(avg_all, columns=["power(W)"])
df.to_csv(base_dir + 'util_csv/' + testcase + '.csv', index=False)
