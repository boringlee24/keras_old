...
for tc in dirs:
    ...
    for fil in files:
        ...
        try: # in case the file is empty
            ...
            pwr = data[data.columns[4]].tolist()
    ...
    avg_pwr_filter = [i for i in avg_pwr if i > 0] # remove power measurements below 10W
    avg_all.append(np.median(avg_pwr_filter))

df = pandas.DataFrame(avg_all, columns=["power(W)"])
df.to_csv(base_dir + 'regression/mem_util/' + testcase + '.csv', index=False)
