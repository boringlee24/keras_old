...
#SBATCH --qos=high
##SBATCH --exclusive
##SBATCH --exclude=d-12-12-1

#srun ./run.sh $TESTCASE & python ${NETWORK}.py --tc $TESTCASE --model $ARCH -b $BATCH --lr 0.001
srun ./dcgmi.sh & python ${NETWORK}.py --tc $TESTCASE --model $ARCH -b $BATCH --lr $LR
#srun python ${NETWORK}.py --tc $TESTCASE --model $ARCH -b $BATCH --lr $LR && ./dcgmi_collect.sh 
